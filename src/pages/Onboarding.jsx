...
const handleGenerate = async () => {
  ...
  try {
    ...
    // The database stores weight_lbs. Convert metric input before saving.
    ...
    // OpenRouter structured-output schemas must be valid for strict JSON Schema.
    // Keep the onboarding schema intentionally simple at the boundary; the AI
    // prompt already contains the detailed training structure requirements.
    const structureSchema = {
      ...
      required: ['program_name', 'duration_weeks', 'macrocycle', 'mesocycles'],
    };

    setProgress(18);
    setLoadingPhase('Designing your program structure…');

    const structure = await supabaseApi.ai.invoke({
      type: 'structure',
      prompt: buildStructurePrompt(trainingType, promptData),
      schema: structureSchema,
    });
    if (!structure || typeof structure !== 'object') {
      throw new Error('AI returned an invalid program structure.');
    }

    const mesocycles = Array.isArray(structure.mesocycles) ? structure.mesocycles : [];
    if (!mesocycles.length) {
      throw new Error('AI returned no training phases. Please try again.');
    }

    setProgress(30);
    setLoadingPhase(`Structure complete — ${mesocycles.length} training phases found.`);

    const microcycleSchema = {
      type: 'object',
      additionalProperties: false,
      properties: {
        microcycles: { type: 'array' },
      },
    ...
    const programPayload = {
      ...
      macrocycle: structure.macrocycle,
    ...
    if (!savedProgram) throw new Error('The program was generated but could not be confirmed as saved.');
    ...
    setProgress(100);
    setLoadingPhase('Your personalized program is ready!');
    ...
    const message = error?.message || error?.details || error?.hint || 'Failed to generate your program. Please try again.';
    toast.error(message);
    setProgress(0);
    setLoadingPhase('');
    setLoading(false);
