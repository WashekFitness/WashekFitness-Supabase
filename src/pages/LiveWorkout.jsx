...
/*
 * IMPORTANT:
 * Numeric inputs intentionally keep their value as a string while editing.
 * This allows the user to completely erase the value without React immediately
 * putting "0" back into the field.
 */
...
function MovementBaseline({ trainingType }) {
  ...
  const { data: baselines = [] } = useQuery({
    queryKey: ['baselines'],
    queryFn: () =>
      supabaseApi.entities.MovementBaseline.list('-recorded_date', 1),
  });
  ...
  const saveBaseline = async () => {
    ...
    ALL_BASELINE_MOVEMENTS.forEach(m => {
      /*
       * Do not use truthiness to decide whether a number exists.
       * An empty string means the user intentionally cleared the field.
       */
      if (values[m.key] !== '' && values[m.key] != null) {
        const parsed = parseFloat(values[m.key]);
    ...
    <div className="space-y-4 pb-4">
      <div className="bg-muted/40 rounded-2xl p-4 border border-border">
        <p className="text-xs text-muted-foreground leading-relaxed">
          ...
          <strong>doesn't affect your live workout</strong> — it's just so
          Kael knows where you're at and can program smarter.
        </p>
      </div>
  ...
  return (
    <Card className="p-4 space-y-3">
      ...
      <p className="text-xs text-muted-foreground">
        How did it go? Any pain, anything that felt too easy or too
        hard, energy levels — just tell Kael in your own words.
      </p>
...
export default function LiveWorkout() {
  ...
  const logMutation = useMutation({
    ...
    onError: (_err, _data, ctx) => {
      if (ctx?.prev) {
        queryClient.setQueryData(['logs'], ctx.prev);
      }
    },
  ...
  const updateLogMutation = useMutation({
    mutationFn: ({ id, data }) =>
      supabaseApi.entities.WorkoutLog.update(id, data),
  ...
  // The Live Workout Tracker itself is free for every plan.
  // Only real-time/dynamic program adjustments remain Elite-only.
    ...
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <div className="px-5 safe-top pb-3 border-b border-border bg-card/50 backdrop-blur-sm flex-shrink-0 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">
            Week {currentWeekNum} · {program.program_name}
          </p>
        </div>
            ...
            <div className="space-y-4">
              <Card className="p-4 text-center border-accent/30 bg-accent/5">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  {fmt(seconds)} · {exercises.length} exercises
                </p>
              </Card>
                ...
                <PostWorkoutCheckin
                  ...
                  onSave={async (checkin, aiNote) => {
                    await updateLogMutation.mutateAsync({
                      id: savedLog.id,
                      data: {
                        post_workout_checkin: checkin,
                        ai_adjustment_notes: aiNote,
                      },
                    });
                  }}
                />
          ...
          {started &&
            ...
            !resting && (
              <Card className="p-5 border-2 border-primary/30">
                ...
                <p className="text-sm text-muted-foreground mb-1">
                  Target: {exercise.reps} reps · {restSecs}s rest
                </p>
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <Input
                    ...
                    onChange={e =>
                      setRepsAchieved(prev => ({
                        ...prev,
                        [exIndex]: e.target.value,
                      }))
                    }
                  />
                </div>
