...
function normalizeUser(
  ...
  return {
     ...
     * Auth user metadata is not a trusted authorization source.
     */
    role:
      profile.role ||
      'user',
...
async function uploadFile(
  ...
  /*
   * Verify that Storage can actually see the object before
   * trying to create a signed URL.
   *
   * This prevents the generic "object not found" error from
   * hiding a failed/misconfigured Storage upload.
   */
  const objectName =
    path.split('/').pop();
  ...
  if (verifyError) {
    throw errorFrom(
      verifyError,
      'The file uploaded, but Storage could not verify the object.'
    );
  }
  ...
  if (!objectExists) {
    throw new Error(
      'The file upload completed, but the uploaded object could not be found in Storage.'
    );
  }

  /*
   * The bucket is private, so never use getPublicUrl().
   *
