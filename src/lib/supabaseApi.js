...
const MEDIA_BUCKET = import.meta.env.VITE_SUPABASE_MEDIA_BUCKET || 'user-media';
const AI_FUNCTION = import.meta.env.VITE_SUPABASE_AI_FUNCTION || 'ai-generate';
const EMAIL_FUNCTION = import.meta.env.VITE_SUPABASE_EMAIL_FUNCTION || 'send-contact-email';
...
async function requireUser() {
  ...
  if (error) throw errorFrom(error, 'Unable to read the signed-in user.');
...
async function getProfile(userId) {
  ...
  if (error) throw errorFrom(error, 'Unable to load your profile.');
  return data || {};
}
...
function normalizeUser(authUser, profile) {
  ...
  const fullName = profile.full_name || [firstName, lastName].filter(Boolean).join(' ') || authUser.user_metadata?.full_name || authUser.email || 'Athlete';
  return {
    id: authUser.id,
...
async function currentUser() {
  const { data, error } = await supabase.auth.getUser();
  if (error) throw errorFrom(error, 'Unable to read authentication state.');
  if (!data?.user) throw new Error('Not authenticated.');
  return normalizeUser(data.user, await getProfile(data.user.id));
}
...
function applyFilters(query, filters = {}, userId) {
  ...
  let ownerFilterSeen = false;
  for (const [key, value] of entries) {
    if (key === 'created_by') {
      ownerFilterSeen = true;
      // Legacy owner filters are translated to the authenticated Supabase user id.
      continue;
    }
  ...
  // Every entity in this app is user-owned. This prevents cross-user reads even
  // if an old page forgot to include an owner filter.
...
function entity(table) {
  return {
    async list(sort = '-created_at', limit = 100) {
      ...
      const column = ORDER_ALIASES[sort.replace(/^-/, '')] || sort.replace(/^-/, '');
...
async function uploadFile(input, folder = 'uploads') {
  ...
  // The bucket should be public so saved photo/video URLs remain usable. If it is
  // private, replace this with createSignedUrl in the app's storage policy.
  const { data } = supabase.storage.from(MEDIA_BUCKET).getPublicUrl(path);
  if (!data?.publicUrl) throw new Error('The file uploaded, but no public URL was returned.');
  return { file_url: data.publicUrl, path };
}

async function invokeAI({ prompt, file_urls = [], model = null, response_json_schema = null, schema = null, type = 'general' } = {}) {
  ...
  if (error) throw errorFrom(error, 'AI generation failed.');
  const result = data?.result ?? data;
  if (result == null) throw new Error('AI generation returned no result.');
  if (result?.success === false) throw new Error(result.error || 'AI generation failed.');
  return result;
}

async function sendEmail(payload) {
  const { data, error } = await supabase.functions.invoke(EMAIL_FUNCTION, { body: payload });
...
export const supabaseApi = {
  auth: {
    ...
    updateMe: async (patch) => {
      const user = await requireUser();
      const profilePatch = { ...patch, id: user.id };
      const { data, error } = await supabase.from('profiles').upsert(profilePatch, { onConflict: 'id' }).select().single();
